...
function compareObjects(obj1, obj2) {
  ...
  return (
    Object.entries(obj2).length === obj1Entries.length &&
    obj1Entries.every(([key, value]) => value === obj2[key])
  );
}
...
class Selector {
  ...
  id(value) {
    if (this.#lastSelector === 'id') {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (this.#lastSelector !== 'element' && this.#lastSelector !== '') {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
  ...
  pseudoClass(value) {
    if (this.#lastSelector === 'pseudoElement') {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
  ...
  combine(combinator, selector) {
    this.#parts = this.#parts.concat(`${combinator}`, selector.#parts);
    this.#lastSelector = selector.#lastSelector;
    return this;
  }
